...
export class YouTubeProcessor extends WorkerHost {
  ...
  private async handleYouTubeUpload(data: YouTubeUploadJobData): Promise<void> {
    ...
    this.logger.log(`Processing YouTube upload — publicationId=${publicationId}`);
    ...
    try {
      ...
      if (!clip.r2ObjectKey) {
        throw new Error('Clip has no R2 object key — it may not have been processed yet.');
      }

      // Load YouTube connection (with tokens)
      const connection = await this.youtubeOAuthService.findConnectionWithTokens(userId);
      if (!connection) {
        throw new Error('YouTube connection not found. User may have disconnected.');
      }

      // Get a valid (possibly refreshed) access token
      const accessToken = await this.youtubeOAuthService.getValidAccessToken(connection);
      ...
      const result = await this.youtubeApiService.uploadVideo(
        accessToken,
        {
          ...
          privacyStatus: publication.privacyStatus as 'private' | 'unlisted' | 'public',
        },
        videoStream,
        contentLength,
      );
  ...
  private getSafeErrorMessage(err: any): string {
    ...
    if (youtubeError) {
      ...
      const ytMessage = youtubeError?.message || youtubeError?.errors?.[0]?.message;
      if (ytMessage) {
        return `YouTube API error: ${ytMessage}`;
      }
    }
    if (err?.message && typeof err.message === 'string' && err.message.length < 300) {
      return err.message;
    }
