...
async function bootstrapActivitiesWorker() {
  ...
  const app = await NestFactory.createApplicationContext(ActivitiesWorkerModule);
  app.enableShutdownHooks();

  const processRegistry = app.get(ProcessRegistryService);

  const shutdown = async (signal: string) => {
    logger.log(`Received ${signal}, shutting down activities worker gracefully...`);
  ...
  logger.log('Activities worker started — listening for jobs on the activities queue.');
}
