...
export const HighlightSchema = z.object({
  ...
  emojis: z
    ...
    .optional()
    .default([])
    .describe('1-3 contextual emojis that match this moment and amplify reaction (e.g. ["🤯", "🔥"])'),
});

export const HighlightsResponseSchema = z.object({
  videoTitle: z.string().max(150).optional().default('').describe('Punchy, clickable title under 100 characters'),
  videoDescription: z.string().max(600).optional().default('').describe('2-4 sentences, max 500 characters, naturally weaving in relevant keywords and hashtags'),
  keywords: z.string().max(600).optional().default('').describe('Comma-separated SEO keywords/phrases for the whole video, max 500 characters total'),
  hashtags: z.array(z.string().max(50)).optional().default([]).describe('5-15 hashtags, each starting with #, lowercase, no spaces'),
...
export class HighlightDetectionService {
  ...
  constructor(private configService: ConfigService) {
    this.provider = this.configService.get<string>('LLM_PROVIDER', 'google').toLowerCase();
    ...
    this.logger.log(`HighlightDetectionService initialized — provider=${this.provider}, model=${this.resolvedModelName}`);
  }
  ...
  private async waitForBudget(estimatedTokens: number): Promise<void> {
    ...
    for (; ;) {
      ...
      const msUntilOldestExpires = oldest ? oldest.timestamp + WINDOW_MS - Date.now() : 1000;
  ...
  private estimateRequestTokens(segments: TranscriptSegmentDto[]): number {
    const transcriptChars = segments.reduce((sum, s) => sum + s.text.length + 20, 0);
  ...
  async detectHighlightsWithMetadata(
    ...
    const filtered = segments.filter((s) => s.text.trim().length >= MIN_SEGMENT_CHARS);
    if (filtered.length === 0) {
      this.logger.warn('No segments remained after filtering filler — falling back to unfiltered segments');
    }
  ...
  private async detectHighlightsDirectWithMetadata(
    ...
    try {
      ...
      const parsedObj = result.object as z.infer<typeof HighlightsResponseSchema>;
      const dtos = this.toDto(parsedObj);
      const merged = this.mergeCandidates(dtos);
      const topHighlights = merged.sort((a, b) => b.score - a.score).slice(0, FINAL_HIGHLIGHT_COUNT);
      ...
      this.logger.error(`Highlight detection failed: ${e instanceof Error ? e.message : e}`);
  ...
  private async detectHighlightsGroqWithMetadata(
    ...
    for (let i = 0; i < chunks.length; i++) {
      this.logger.log(`Processing chunk ${i + 1}/${chunks.length} (${chunks[i].length} segments)`);
      const res = await this.detectHighlightsInChunkWithMetadata(chunks[i], options, duration);
      allCandidates.push(...res.highlights);
      if (!videoTitle && res.videoTitle) videoTitle = res.videoTitle;
      if (!videoDescription && res.videoDescription) videoDescription = res.videoDescription;
      if (!keywords && res.keywords) keywords = res.keywords;
      if (hashtags.length === 0 && res.hashtags?.length) hashtags = res.hashtags;
    }
  ...
  private chunkSegments(segments: TranscriptSegmentDto[]): TranscriptSegmentDto[][] {
    ...
    for (const seg of segments) {
      ...
      if (currentChars + lineChars > CHUNK_CHAR_BUDGET && current.length > 0) {
        chunks.push(current);
        const overlapStart = current[current.length - 1].endTime - CHUNK_OVERLAP_SECONDS;
  ...
  private isRateLimitError(e: any): boolean {
    ...
    return candidates.some((err: any) => {
      const isApiError = err?.constructor?.name === 'AI_APICallError' || err?.name === 'AI_APICallError';
      if (!isApiError) return false;
      return (
        err?.statusCode === 413 ||
        err?.statusCode === 429 ||
        (typeof err?.responseBody === 'string' && err.responseBody.includes('rate_limit_exceeded'))
      );
    });
  }
  ...
  private isConnectionError(e: any): boolean {
    ...
    return (
      ...
      (e?.statusCode === undefined && e?.responseBody === undefined &&
        (e?.constructor?.name === 'AI_APICallError' || e?.name === 'AI_APICallError'))
    );
  }
  ...
  private logFullErrorBody(e: any, label: string): void {
    if (e?.responseBody) {
      ...
      this.logger.error(`${label} — lastError.responseBody: ${e.lastError.responseBody}`);
    } else if (Array.isArray(e?.errors)) {
      e.errors.forEach((err: any, i: number) => {
        if (err?.responseBody) this.logger.error(`${label} — errors[${i}].responseBody: ${err.responseBody}`);
      });
    }
  }

  private buildGenerateObjectOptions(model: any, systemPrompt: string, userPrompt: string, temperature: number) {
    const base: any = {
      model,
      schemaName: 'HighlightsResponse',
      schemaDescription: 'List of video highlights and metadata',
  ...
  private async detectHighlightsInChunk(
    ...
    const res = await this.detectHighlightsInChunkWithMetadata(segments, options, videoDuration, rateLimitRetryCount);
    return res.highlights;
  }
  ...
  private async detectHighlightsInChunkWithMetadata(
    ...
    try {
      const result = await generateObject(
        this.buildGenerateObjectOptions(this.model, systemPrompt, userPrompt, 0.3),
      );
      ...
      if (this.isRateLimitError(e)) {
        ...
        if (rateLimitRetryCount >= MAX_RATE_LIMIT_RETRIES) {
          this.logger.error(`Chunk hit TPM rate limit and exhausted ${MAX_RATE_LIMIT_RETRIES} retries — skipping this chunk.`);
          return { videoTitle: '', videoDescription: '', keywords: '', hashtags: [], highlights: [] };
        }
        this.logger.warn(
          `Chunk hit TPM rate limit (attempt ${rateLimitRetryCount + 1}/${MAX_RATE_LIMIT_RETRIES}) — will re-check budget and retry`,
        );
        return this.detectHighlightsInChunkWithMetadata(segments, options, videoDuration, rateLimitRetryCount + 1);
      }
      ...
      if (isLengthError || isSchemaError) {
        ...
        try {
          const retryResult = await generateObject(
            this.buildGenerateObjectOptions(this.model, systemPrompt, userPrompt, 0.1),
          );
          this.recordUsage(estimatedTokens);
          const parsedRetry = retryResult.object as z.infer<typeof HighlightsResponseSchema>;
          return {
          ...
          if (this.isRateLimitError(retryError) && rateLimitRetryCount < MAX_RATE_LIMIT_RETRIES) {
            this.logger.warn(
          ...
          if (this.isConnectionError(retryError) && rateLimitRetryCount < MAX_RATE_LIMIT_RETRIES) {
            this.logger.warn(
          ...
          this.logger.error(`Chunk retry also failed: ${retryError instanceof Error ? retryError.message : retryError}`);
          return { videoTitle: '', videoDescription: '', keywords: '', hashtags: [], highlights: [] };
        }
      } else if (NoObjectGeneratedError.isInstance(e)) {
        this.logger.error(`Chunk: no object generated. Finish reason: ${e.finishReason}`);
        ...
        this.logger.error(`Chunk connection error persisted after ${MAX_RATE_LIMIT_RETRIES} retries — skipping: ${e}`);
        return { videoTitle: '', videoDescription: '', keywords: '', hashtags: [], highlights: [] };
      } else {
        if (e?.constructor?.name === 'AI_APICallError' || e?.name === 'AI_APICallError') {
          this.logger.error(`Chunk AI_APICallError — status: ${e.statusCode}, responseBody: ${e.responseBody}`);
        } else if (e?.name === 'AI_RetryError' || e?.name === 'RetryError') {
          this.logger.error(`Chunk RetryError — lastError: ${e?.lastError?.message ?? e?.message}`);
  ...
  private toDto(object: z.infer<typeof HighlightsResponseSchema>): HighlightDto[] {
    return (object.highlights || [])
      ...
      .map((h, index) => {
        ...
        return {
          ...
          tags: (h.tags ?? []).map((t) => t.trim().toLowerCase().replace(/^#+/, '')).filter(Boolean),
  ...
  private mergeCandidates(candidates: HighlightDto[]): HighlightDto[] {
    ...
    for (const candidate of sorted) {
      const overlapsExisting = kept.some((existing) => {
        ...
        return candidateDuration > 0 && overlapDuration / candidateDuration > 0.5;
      });
